...
public class Line2Controller : MonoBehaviour
{
    ...
    Rigidbody otherRigidbody = other.attachedRigidbody;
        if (otherRigidbody != null && dicRB.ContainsKey(otherRigidbody)){
            if (dicRB[otherRigidbody]) {
                line2.SetPosition(1, new Vector3(other.transform.position.x, other.transform.position.y, other.transform.position.z - 0.5f));
                isFire = true;
            }
        }
    }
